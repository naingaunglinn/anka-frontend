    ...
    <main className="bg-[#f8fafc] text-[#171717]">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-[#e6e9ee] bg-[#f8fafc]/85 backdrop-blur-xl">
        <div className="mx-auto flex h-20 w-full max-w-7xl items-center justify-between px-6 md:px-10">
          <div className="flex items-center gap-3 text-xl font-semibold tracking-tight">
            <span className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[#00a6f4]/35 text-[#00a6f4]">A</span>
            ANKA
          </div>
          <nav className="hidden items-center gap-8 text-sm text-[#4a4a4a] md:flex">
            <a href="#features" className="hover:text-[#00a6f4]">Features</a>
            <a href="#how" className="hover:text-[#00a6f4]">How it works</a>
            <a href="#pricing" className="hover:text-[#00a6f4]">Pricing</a>
            <a href="#docs" className="hover:text-[#00a6f4]">Documentation</a>
          </nav>
      ...
      <section className="relative overflow-hidden px-6 pb-20 pt-36 md:px-10 md:pt-44">
        ...
        <div className="mx-auto grid w-full max-w-7xl items-center gap-14 lg:grid-cols-2">
          <div>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-[#00a6f4]/10 px-4 py-2 text-xs uppercase tracking-[0.18em] text-[#0085c4]">
              <span className="h-2 w-2 rounded-full bg-[#00a6f4]" /> Gross Profit Intelligence · v3.2
            </div>
            <h1 className="text-5xl font-semibold leading-[0.95] tracking-[-0.03em] md:text-7xl">
              Price with conviction.<br />
              <span className="text-[#00a6f4]">Profit with precision.</span>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href="/register" className="inline-flex items-center gap-2 rounded-full bg-[#171717] px-6 py-3 text-sm font-semibold text-white hover:bg-[#00a6f4]">
                Start free trial <ArrowRight className="h-4 w-4" />
              </Link>
              <Link href="/login" className="inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold text-[#171717] hover:text-[#00a6f4]">
                Watch 2-min demo
              </Link>
            </div>
          ...
          <div className="relative h-[520px]">
            <div className="absolute left-0 right-10 top-0 rounded-2xl border border-[#e6e9ee] bg-white p-6 shadow-[0_20px_60px_-20px_rgba(23,23,23,0.12)]">
              <div className="mb-3 flex items-center justify-between text-xs uppercase tracking-[0.14em] text-[#8a8a8a]">
                ...
                <span className="rounded-full bg-[#00a6f4]/10 px-3 py-1 text-[#0085c4]">LIVE</span>
              </div>
              <div className="mb-1 flex items-end gap-3">
                <p className="text-5xl font-semibold tracking-tight">$4.82M</p>
                <p className="flex items-center gap-1 font-mono text-sm font-semibold text-[#00a6f4]"><TrendingUp className="h-4 w-4" />+14.2%</p>
              </div>
            ...
            <div className="absolute bottom-5 right-0 w-56 rounded-2xl border border-[#e6e9ee] bg-white p-4 shadow-[0_20px_60px_-20px_rgba(23,23,23,0.12)]">
              ...
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-[#eef1f5]"><div className="h-full w-[78%] bg-[#00a6f4]" /></div>
      ...
      <section id="features" className="mx-auto w-full max-w-7xl px-6 py-24 md:px-10">
        <p className="mb-3 font-mono text-xs uppercase tracking-[0.18em] text-[#0085c4]">What ANKA does</p>
      ...
      <section id="how" className="bg-[#171717] px-6 py-24 text-[#f8fafc] md:px-10">
        <div className="mx-auto w-full max-w-7xl">
          <p className="mb-3 font-mono text-xs uppercase tracking-[0.18em] text-[#00a6f4]">The flow</p>
      ...
      <section className="px-6 py-24 text-center md:px-10" id="pricing">
        ...
        <div className="mt-9 flex flex-wrap justify-center gap-3">
          <Link href="/register" className="inline-flex items-center gap-2 rounded-full bg-[#171717] px-6 py-3 text-sm font-semibold text-white hover:bg-[#00a6f4]">Book a margin assessment <ArrowRight className="h-4 w-4" /></Link>
          <a href="#docs" className="inline-flex items-center rounded-full px-6 py-3 text-sm font-semibold text-[#171717] hover:text-[#00a6f4]">Read the technical brief</a>
        </div>
      </section>
...
function Suggestion({ t, m, i }: { t: string; m: string; i: string }) {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-[#eef1f5] bg-[#f8fafc] p-3">
      <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-[#00a6f4]/12 text-[#00a6f4]">$</span>
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium">{t}</p>
        <p className="truncate text-xs text-[#8a8a8a]">{m}</p>
      </div>
      <p className="font-mono text-xs font-semibold text-[#00a6f4]">{i}</p>
    </div>
  );
}
...
function Card({ icon, t, d, className = "" }: { icon: React.ReactNode; t: string; d: string; className?: string }) {
  return (
    <article className={`rounded-2xl border border-[#e6e9ee] bg-white p-7 shadow-[0_12px_36px_-20px_rgba(0,166,244,0.25)] ${className}`}>
      <div className="mb-4 inline-flex rounded-xl bg-[#00a6f4]/10 p-3 text-[#00a6f4]">{icon}</div>
...
function Step({ title, text }: { title: string; text: string }) {
  return (
    <article className="border-l border-[#00a6f4]/35 pl-5">
      <p className="mb-5 font-mono text-xs uppercase tracking-[0.14em] text-[#00a6f4]">Step</p>
  ...
  return (
    <div>
      ...
      <ul className="space-y-2 text-sm text-[#171717]">
        {items.map((item) => (
          <li key={item}><a href="#" className="hover:text-[#00a6f4]">{item}</a></li>
        ))}
      </ul>
    </div>
  );
