...
export function DepartmentsTable({ data, onEdit, onDelete }: DepartmentsTableProps) {
    ...
    const columns: ColumnDef<Department>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => <div className="text-right">{row.getValue('headcount')}</div>,
        },
        {
            id: 'actions',
            cell: ({ row }) => {
                        ...
                        {table.getHeaderGroups().map((headerGroup) => (
                            <TableRow key={headerGroup.id}>
                                {headerGroup.headers.map((header) => {
                                    return (
                                        <TableHead key={header.id} className={header.column.id === 'headcount' ? 'text-right' : ''}>
                            ...
                            table.getRowModel().rows.map((row) => (
                                <TableRow key={row.id} data-state={row.getIsSelected() && 'selected'}>
                                    {row.getVisibleCells().map((cell) => (
                                        <TableCell key={cell.id} className={cell.column.id === 'headcount' ? 'text-right' : ''}>
